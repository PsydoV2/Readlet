...
- `react-native-webview` — renders EPUB chapters and PDFs (see Reader section for why, over `react-native-epub-view`/`react-native-pdf`)
...
- **Import** (`src/services/importBook.ts`, called via `useLibrary().importBook()`): opens `expo-document-picker` restricted to `application/epub+zip`/`application/pdf`, copies the picked file into `Paths.document/books/<id>.<ext>` via the new class-based `expo-file-system` API (`File`/`Directory`/`Paths` — **not** the legacy `FileSystem.documentDirectory` function API; SDK 57 ships both, this codebase uses the new one), then branches by format:
  ...
  - Both: a deterministic placeholder cover color from `src/utils/accentColor.ts` (hashes the book id against a fixed palette) — there's no real cover-art extraction yet (EPUBs do list a cover image in their manifest; PDFs would need a rendered first-page thumbnail).
- **Deleting** a book (`useLibrary().removeBook`) removes the DB row and best-effort deletes the copied source file + (for EPUBs) the extracted folder (`deleteBookFiles` in `importBook.ts`) — failures there are swallowed, since a missing/already-gone file should never block removing the library entry.
- **Progress** is chapter-based for EPUBs (`currentPosition` = index into `spine`, `progress` = `currentPosition / (spine.length - 1)`) and **not currently tracked** for PDFs — see the Reader section for why.
...
- **EPUB**: loads the current spine chapter's already-extracted XHTML file straight from disk (`${book.extractedDir}/${book.spine[i]}`, a `file://` URI) with `injectedJavaScript` adding a `<style>` tag (via `textContent`, not `innerHTML`) that reads/font-sizes/colors match the app theme. "Chapter" is the unit of both navigation (prev/next buttons in the bottom bar) and progress — there's no in-chapter pagination or scroll-position tracking. `WebView` needs `allowFileAccess`/`allowFileAccessFromFileURLs`/`allowUniversalAccessFromFileURLs` (Android) and `allowingReadAccessToURL={book.extractedDir}` (iOS) to load local files and let a chapter resolve its own relative image/CSS references.
...
- The preference is **not persisted** yet — it resets to `"system"` on app restart. Wire it up to local storage once one exists rather than reaching for `@react-native-async-storage/async-storage`, which was deliberately removed (see No-auth stance above). (Contrast with the language preference below, which *is* persisted, via `expo-secure-store` — reasonable to bring theme in line with that the next time this file is touched.)
...
- Every user-facing string in every screen goes through `useTranslation()`'s `t()` — including toasts and `Error` messages thrown from `importBook.ts`'s top-level "unsupported format" case (via `i18next.t(...)`, since that's outside a component). The one deliberate exception: `epubService.ts`'s deep parser-validation errors (malformed `container.xml`/OPF) stay hardcoded German — rare, technical, developer-facing-in-practice failures, not worth a translation key. When adding new user-facing text anywhere, add a key to **both** `src/i18n/locales/de.json` and `en.json` rather than hardcoding a string — `de` is `fallbackLng`, so a missing `en` key silently falls back to German rather than erroring, which makes a forgotten translation easy to miss.
